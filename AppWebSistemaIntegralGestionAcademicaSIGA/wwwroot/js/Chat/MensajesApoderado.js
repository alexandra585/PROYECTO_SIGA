(function () {
                ...
                try {
                    ...
                    if (resp.ok) {
                        ...
                        (data.usuarios || []).forEach(function (id) {
                            document.querySelectorAll('.estado-online[data-docente-id="' + id + '"]').forEach(function (p) {
                                p.classList.remove('bg-slate-300');
                                p.classList.add('bg-emerald-500');
                            });
                            if (docenteActualId === id) {
    ...
    function onUsuarioEnLinea(usuarioId) {
        document.querySelectorAll('.estado-online[data-docente-id="' + usuarioId + '"]').forEach(function (p) {
            p.classList.remove('bg-slate-300');
            p.classList.add('bg-emerald-500');
        });
        if (docenteActualId === usuarioId) {
            var el = document.getElementById('chatEstadoOnline');
            if (el) {
                el.classList.remove('bg-slate-300');
                el.classList.add('bg-emerald-500');
    ...
    function onRecibirMensaje(mensaje) {
        ...
        var item = document.querySelector('.docente-item[data-conversacion-id="' + convId + '"]');
        if (item) {
    ...
    function onActualizarUltimoMensaje(conversacionId, texto) {
        var item = document.querySelector('.docente-item[data-conversacion-id="' + conversacionId + '"]');
        if (item) {
            var ultimo = item.querySelector('.ultimo-mensaje');
    ...
    function mostrarEstadoVacio() {
        ...
        cont.innerHTML =
            ...
            '<p class="mt-2 max-w-sm text-sm leading-relaxed text-slate-500">Redacta tu primer mensaje para iniciar la conversación con este docente. Estará pendiente de tu consulta.</p>' +
            '</div>';
    }
    ...
    async function abrirConversacion(docenteId, nombre, especialidad) {
        if (cargandoConversacion) return;
        cargandoConversacion = true;
        docenteActualId = docenteId;
        document.getElementById('chatVacio').classList.add('hidden');
        document.getElementById('chatActivo').classList.remove('hidden');
        document.getElementById('chatNombreDocente').textContent = nombre;
        document.getElementById('chatEspecialidad').textContent = especialidad || 'Docente';
        var estadoItem = document.querySelector('.estado-online[data-docente-id="' + docenteId + '"]');
        ...
        try {
            var itemLista = document.querySelector('.docente-item[data-docente-id="' + docenteId + '"]');
            var convExistente = itemLista ? itemLista.getAttribute('data-conversacion-id') : '';
            var url = '/Chat/ObtenerConversacionApoderado?docenteId=' + docenteId;
        ...
        var item = document.querySelector('.docente-item[data-docente-id="' + docenteId + '"]');
    ...
    async function cerrarChat() {
        ...
        cargandoConversacion = false;
        document.querySelectorAll('.docente-item').forEach(function (b) {
    ...
    function eliminarChat() {
        ...
        abrirModal(
            'Eliminar chat',
            '¿Eliminar este chat? Desaparecerá de tu lista. El docente seguirá viendo la conversación.',
            true,
            async function () {
                try {
    ...
    function vaciarChat() {
        ...
        abrirModal(
            'Vaciar chat',
            '¿Vaciar el chat? Solo se borrarán los mensajes para ti. El docente no se verá afectado.',
            false,
            async function () {
                try {
    ...
    function filtrarDocentes() {
        var q = (document.getElementById('buscarDocente').value || '').toLowerCase();
        document.querySelectorAll('.docente-item').forEach(function (item) {
            var nombre = (item.getAttribute('data-nombre') || '').toLowerCase();
            var esp = (item.getAttribute('data-especialidad') || '').toLowerCase();
            item.style.display = (nombre.indexOf(q) !== -1 || esp.indexOf(q) !== -1) ? '' : 'none';
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.docente-item').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                abrirConversacion(
                    parseInt(btn.getAttribute('data-docente-id'), 10),
                    btn.getAttribute('data-nombre'),
                    btn.getAttribute('data-especialidad')
                );
            });
        });
        ...
        var buscar = document.getElementById('buscarDocente');
        buscar.addEventListener('input', filtrarDocentes);
