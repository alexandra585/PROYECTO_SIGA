document.addEventListener('DOMContentLoaded', function () {
    ...
    function buscarUsuarios(filtro) {
        ...
        fetch(`/CRUDApoderado/BuscarUsuarioDisponibleApoderado?filtro=${encodeURIComponent(filtro)}`)
            .then(r => {
            ...
            listaUsuarios.innerHTML = `
                <div class="flex h-full flex-col items-center justify-center px-6 py-8 text-center">
                    ...
                    <p class="mt-2 max-w-xs text-sm text-slate-500">
                        Registre primero un usuario con rol <strong class="text-slate-700">Apoderado</strong> para poder vincularlo y registrarlo en el sistema.
                    </p>
            ...
            html += `
                <div class="flex cursor-pointer items-center justify-between rounded-xl px-4 py-3 hover:bg-emerald-50"
                    ...
                    <div class="flex items-center gap-2">
                        <span class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700">${u.rol || 'Apoderado'}</span>
    ...
    document.getElementById('formApoderado').addEventListener('submit', function (e) {
        if (modoExistente.checked) {
